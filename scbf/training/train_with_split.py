...
FIX (v2): Embeddings are now L2-normalized before computing distances.
Previously the contrastive loss had no normalization, so the model could
minimize loss simply by scaling up the norm of ALL embeddings uniformly
(both clean and malicious grew together), which looked like "improvement"
in raw distance numbers but did not actually improve class separability.
Normalizing onto the unit hypersphere removes that escape hatch and forces
the loss to improve genuine angular/relative separation. Margin has been
rescaled to match the 0-2 range of distances between unit vectors.
"""
...
def compute_embeddings(model, data_paths, normalize=True):
    """
    ...
    Args:
        ...
        normalize: If True (default), L2-normalize each embedding onto the
            unit hypersphere. This is the key fix: without normalization the
            model can trivially reduce the contrastive loss by scaling up
            the norm of every embedding (clean and malicious alike), which
            inflates "mal distance" without improving actual separability.
...
def compute_contrastive_loss(embeddings, labels, margin=0.8):
    ...
    Assumes `embeddings` are already L2-normalized (unit vectors), so all
    pairwise distances lie in [0, 2]. This makes `margin` interpretable and
    achievable: margin=1.0 roughly corresponds to orthogonal vectors,
    margin=2.0 to antipodal vectors. Previously margin=3.0 was set against
    unnormalized embeddings with typical distances of ~0.6, meaning the
    margin was essentially unreachable and gave the model a permanent,
    unresolvable gradient pressure -- which just pushed it to inflate norms
    instead of learning real separation.
    ...
    Loss = compactness + 3.0 * max(0, margin - mal_dist) + 2.0 * max(0, margin - gap)

    Args:
        embeddings: Tensor of shape (N, embedding_dim), assumed L2-normalized
        labels: Tensor of shape (N,) with 0=clean, 1=malicious
        margin: Margin for contrastive loss (interpreted in normalized-distance units)
    ...
    if clean_mask.sum() > 1:
        ...
        # Re-normalize the centroid so it too lives on (or near) the unit
        # sphere; this keeps distance-from-centroid comparable to the
        # margin, which is calibrated in normalized-distance units.
        clean_centroid = F.normalize(clean_centroid, p=2, dim=-1, eps=1e-8)
        ...
        if mal_mask.sum() > 0:
            ...
            metrics['ratio_mal_to_compact'] = (mal_dist.mean() / (compactness + 1e-8)).item()
        ...
        metrics['ratio_mal_to_compact'] = 0.0
...
def main():
    ...
    VAL_RATIO = 0.15
    TEST_RATIO = 0.15
    # FIX: margin rescaled for L2-normalized embeddings, where all pairwise
    # distances lie in [0, 2]. margin=3.0 was unreachable against
    # unnormalized embeddings sitting around ~0.6 distance -- that gave a
    # permanent, unresolvable gradient that just pushed the model to scale
    # up embedding norms rather than learn real separation. 1.0 corresponds
    # roughly to orthogonal embeddings, which is a reasonable, achievable
    # target for "well separated" classes on the unit hypersphere.
    MARGIN = 1.0
    LEARNING_RATE = 5e-4
    EPOCHS = 20  # Reduced from 60 - was too long
    ...
    for epoch in range(EPOCHS):
        ...
        if 'ratio_mal_to_compact' in val_metrics:
            # This is the real signal of separability: mal distance relative
            # to how tight the clean cluster is. Watch THIS number, not the
            # raw compactness/mal-distance values, which can rise together
            # without the classes actually separating.
            print(f"  Val Separation Ratio (mal/compact): {val_metrics['ratio_mal_to_compact']:.4f}")
    ...
    if 'ratio_mal_to_compact' in test_metrics:
        print(f"Test Separation Ratio (mal/compact): {test_metrics['ratio_mal_to_compact']:.4f}")
    print("=" * 80)
